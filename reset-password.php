...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($password) || strlen($password) < 6) {
        $errors[] = "Please enter a new password with at least 6 characters.";
    }
    ...
    // Update password
    $result = $profileManager->updateProfilePassword($_SESSION['profile_id'], $password);
    if (str_contains($result, "successfully")) {
        // Password updated successfully, destroy the session, redirect to login
        session_destroy();
...
<head>
    ...
    <title>Reset Password | Interactive Cares</title>
                        ...
                        if (!empty($errors)) {
                            foreach ($errors as $error) {
                                if (str_contains($error, 'match')) {
                                    echo '<span class="mt-2 text-sm text-red-600">' . htmlspecialchars($error) . '</span>';
                                } elseif (str_contains($error, 'characters')) {
                                    echo '<span class="mt-2 text-sm text-red-600">' . htmlspecialchars($error) . '</span>';
                                }
                            }
                        }
