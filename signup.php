...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  require_once __DIR__ . '/utilities/ProfileManager.php';
  $profileManager = new ProfileManager();

  // Check if email already exists
  $profile = $profileManager->findProfileByEmail($email);
  if ($profile) {
  ...
  if (empty($errors)) {
    ...
    $result = $profileManager->addProfile([
      'full_name' => $fullName,
      'email' => $email,
      'password' => $password
    ]);
...
<body class="bg-gradient-to-br from-indigo-50 via-white to-cyan-50 min-h-screen flex items-center justify-center p-4">
  <div class="max-w-md w-full space-y-8">
    ...
    <div class="bg-white glass rounded-2xl shadow-xl p-8 transition-all duration-300 hover:shadow-2xl">
      <form class="space-y-6 animate-fadeIn" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>"
        method="POST">
          ...
          <div>
            ...
            <div class="relative">
              ...
              <input type="text"
                ...
                placeholder="John doe" name="fullname" required />
              <?php
              if (!empty($errors)) {
                foreach ($errors as $error) {
                  if (str_contains($error, 'name')) {
                    echo '<span class="mt-2 text-sm text-red-600">' . htmlspecialchars($error) . '</span>';
                  }
                }
              }
              ?>
        ...
        <div>
          ...
          <div class="relative">
            ...
            <?php
            if (!empty($errors)) {
              echo '<span class="mt-2 text-sm text-red-600">';
              foreach ($errors as $error) {
                if (str_contains($error, 'email')) {
                  echo '<p>' . htmlspecialchars($error) . '</p>';
                } elseif (str_contains($error, 'exists')) {
                  echo '<p>' . htmlspecialchars($error) . '</p>';
                }
              }
              echo '</span>';
        ...
        <div>
          ...
          <div class="relative">
            ...
            if (!empty($errors)) {
              echo '<span class="mt-2 text-sm text-red-600">';
              foreach ($errors as $error) {
                if (str_contains($error, 'characters')) {
                  echo '<p>' . htmlspecialchars($error) . '</p>';
                }
              }
              echo '</span>';
        ...
        <div class="flex items-start">
          ...
          <div class="ml-3 text-sm">
            <label for="terms" class="text-gray-700">I agree to the
              <a href="#" class="font-medium text-indigo-600 hover:text-indigo-500 transition-colors">Terms and
                Conditions</a>
              and
    ...
    <div class="text-center text-sm text-gray-600">
      <p>
        ...
        <a href="login.php" class="font-semibold text-indigo-600 hover:text-indigo-500 transition-colors">Sign in</a>
      </p>
    </div>
  </div>
</body>
